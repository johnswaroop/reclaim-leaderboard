...
const Leaderboard = () => {
  ...
  useEffect(() => {
    ...
    let provider = path.split("/")[2];
    setid(provider as string);
  }, []);
  ...
  return (
    <section className="bg-[#00E] min-h-screen flex flex-col w-full h-[880px] max-[1090px]:h-fit relative items-center">
      <Nav />
      <Dots />
      <h1 className="capitalize text-white text-4xl mt-8">
        {providerIds.find((ele) => ele.value == id)?.label}
      </h1>
      {id.length > 0 && <LeaderboardComp id={id} />}
    </section>
  );
};

const LeaderboardComp = ({ id }: { id: string }) => {
  ...
  useEffect(() => {
    axios.get(`/api/leaderboard-proof/${id}`).then((res) => {
